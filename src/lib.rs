...
pub extern "C" fn train_rbfn_model_auto(
    ...
    match &model.mode {
        ...
        RBFMode::MultiClassification(_) => {
            ...
            let x_test = unsafe { std::slice::from_raw_parts(x_test_ptr, n_test * n_features) };
            let y_test = unsafe { std::slice::from_raw_parts(y_test_ptr, n_test * n_outputs) };
            let x_test_rows: Vec<Vec<f64>> = x_test.chunks(n_features).map(|c| c.to_vec()).collect();
            let y_test_rows: Vec<Vec<f64>> = y_test.chunks(n_outputs).map(|c| c.to_vec()).collect();

            model.fit_gradient_descent(&x_rows, &y_rows, &x_test_rows, &y_test_rows);
        }
    }
}
...
#[no_mangle]
pub extern "C" fn save_softmax_model(ptr: *mut SoftmaxModel, path: *const c_char) {
    let model = unsafe { &*ptr };
    let c_path = unsafe { CStr::from_ptr(path) }.to_str().unwrap();
    let file = File::create(c_path).expect("Erreur fichier");
    serde_json::to_writer(file, model).unwrap();
}

#[no_mangle]
pub extern "C" fn load_softmax_model(path: *const c_char) -> *mut SoftmaxModel {
    let c_path = unsafe {
    ...
    let file = match File::open(c_path) {
        Ok(f) => f,
        Err(_) => return std::ptr::null_mut(),
    };

    let model: SoftmaxModel = match serde_json::from_reader(file) {
        Ok(m) => m,
        Err(_) => return std::ptr::null_mut(),
    };
...
#[no_mangle]
pub extern "C" fn save_rbfn_model(ptr: *mut RBFN, path: *const c_char) {
    if ptr.is_null() || path.is_null() {
        return;
    }

    let model = unsafe { &*(ptr as *mut RBFN) };
    let c_str = unsafe { CStr::from_ptr(path) };
    if let Ok(path_str) = c_str.to_str() {
        if let Ok(file) = File::create(path_str) {
            let _ = bincode::serialize_into(file, model);
        }
    }
}
...
pub extern "C" fn load_rbfn_model(path: *const c_char) -> *mut RBFN {
    ...
    if let Ok(path_str) = c_str.to_str() {
        if let Ok(file) = File::open(path_str) {
            if let Ok(model) = bincode::deserialize_from::<_, RBFN>(file) {
                return Box::into_raw(Box::new(model));
            }
        }
    }
...
#[no_mangle]
pub extern "C" fn get_svm_loss_history(
...
#[no_mangle]
pub extern "C" fn get_rbfn_train_losses_ptr(model_ptr: *const RBFN) -> *const f64 {
    unsafe { &(*model_ptr).train_losses }.as_ptr()
}

#[no_mangle]
pub extern "C" fn get_rbfn_train_losses_len(model_ptr: *const RBFN) -> usize {
    unsafe { &(*model_ptr).train_losses }.len()
}

#[no_mangle]
pub extern "C" fn get_rbfn_test_losses_ptr(model_ptr: *const RBFN) -> *const f64 {
    unsafe { &(*model_ptr).test_losses }.as_ptr()
}

#[no_mangle]
pub extern "C" fn get_rbfn_test_losses_len(model_ptr: *const RBFN) -> usize {
    unsafe { &(*model_ptr).test_losses }.len()
}

#[no_mangle]
pub extern "C" fn get_rbfn_train_accuracies_ptr(model_ptr: *const RBFN) -> *const f64 {
    unsafe { &(*model_ptr).train_accuracies }.as_ptr()
}

#[no_mangle]
pub extern "C" fn get_rbfn_train_accuracies_len(model_ptr: *const RBFN) -> usize {
    unsafe { &(*model_ptr).train_accuracies }.len()
}

#[no_mangle]
pub extern "C" fn get_rbfn_test_accuracies_ptr(model_ptr: *const RBFN) -> *const f64 {
    unsafe { &(*model_ptr).test_accuracies }.as_ptr()
}

#[no_mangle]
pub extern "C" fn get_rbfn_test_accuracies_len(model_ptr: *const RBFN) -> usize {
    unsafe { &(*model_ptr).test_accuracies }.len()
}
